...
class LossMonitorMAE(Callback):
    ...
    def __init__(self, per_print_times=1, log=None):
        super(LossMonitorMAE, self).__init__()
...
class LossMonitorMAE_test(Callback):
    """
    Monitor the loss in training.
    ...
    def __init__(self, per_print_times=1, log=None):
        super(LossMonitorMAE_test, self).__init__()
        if not isinstance(per_print_times, int) or per_print_times < 0:
            raise ValueError("The argument 'per_print_times' must be int and >= 0, "
                             "but got {}".format(per_print_times))
        self._per_print_times = per_print_times
    ...
    def on_train_step_end(self, run_context):
        ...
        if self._per_print_times != 0 and (cb_params.cur_step_num - self._last_print_time) >= self._per_print_times:
            ...
            costtime = time.time() - self.time
            self.time = time.time()
            self.log.info("val epoch: %s step: %s, loss is %s,time cost is %.2fs" % (
...
class LossMonitor(Callback):
    ...
    def __init__(self, accum_iter=1, per_print_times=1, ifeval=True, log=None):
