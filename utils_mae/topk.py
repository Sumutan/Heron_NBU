...
def check_softlabel_data(data):
    """
    Whether input data is softlabel encoding.

    Args:
        data (numpy.array): Input data.

    Returns:
        bool, return true, if input data is softlabel encoding.
    """
    if data.ndim > 1 and (abs(np.sum(data,axis=1) - 1) < 1e-4).all():
        return True
    return False
...
class TopKCategoricalAccuracy(Metric):
    ...
    Examples:
        ...
        0.6666666666666666
    """
    def __init__(self, k,logger=None):
    ...
    def clear(self):
        ...
        self._correct_abn_num = 0
        self._samples_abn_num = 0
    ...
    def update(self, *inputs):
        ...
        if y_pred.ndim == y.ndim and _check_onehot_data(y):
            y = y.argmax(axis=1)
        elif y_pred.ndim == y.ndim and check_softlabel_data(y):
            y = y.argmax(axis=1)
        ...
        # abnormal_mask = y == 1
        # self._correct_abn_num += correct[abnormal_mask].sum()
        # self._samples_abn_num += abnormal_mask.sum()
    ...
    def eval(self):
        ...
        self.logger.info(f'top-{self.k} accuracy: {self._correct_num / self._samples_num}')
