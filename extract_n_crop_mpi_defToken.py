...
def assign_tasks(tasks, processors=8):
    ...
    """
    sorted_tasks = sorted(tasks, key=lambda x: x[1], reverse=False)  # reverse=True 推理， False调试
...
def get_run_args():
    ...
    parser.add_argument('--dataset', default="TAD", choices=['UCF-Crime', 'TAD','ShanghaiTech','XD_violence'], type=str)
