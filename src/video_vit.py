...
class PatchEmbed(nn.Cell):
    def __init__(
        ...
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=kernel_size, stride=kernel_size, has_bias=True).to_float(
            ms.float16)

    def construct(self, x):  # [B, C, T, H, W]-> [B,T,N,C]
        B, C, T, H, W = x.shape
        ...
        x = self.proj(x)
        x = x.reshape(B, self.embed_dim, self.input_size[0], self.input_size[1] * self.input_size[2])  # [B,C,T,N]
        x = x.transpose(0, 2, 3, 1)  # [B,T,N,C]
        return x
