...
class VideoDataset:
        ...
        for retry in range(self._num_retries):  # 视频打开失败则重新随机抓取视频
            ...
            try:
                ...
                if len(vr) > 100:
                    ...
                    warning = "视频过短,video id: {}, retries: {}, path: {} length:{}".format(index, retry, sample,
                                                                                              len(vr))
    ...
    def _get_video_cap_cv2(self, index):
        """
        :param index: 视频文件列表的下标索引
        :return:  cv2 cap，文件路径
        """
        for retry in range(self._num_retries):  # 视频打开失败则重新随机抓取视频
            ...
            try:
                cap = cv2.VideoCapture(sample)
                if cap.get(cv2.CAP_PROP_FRAME_COUNT) > 100:
                    return cap, sample, index
                else:
                    warning = "视频过短,video id: {}, retries: {}, path: {} length:{}".format(index, retry, sample,
                ...
                warning = "视频打开失败,video id: {}, retries: {}, path: {}".format(index, retry, sample)
                self.logger.warning(warning.format(index, retry, sample, cap.get(cv2.CAP_PROP_FRAME_COUNT)))
            index = np.random.randint(0, len(self._path_to_videos) - 1)
        raise RuntimeError("数据多次读取失败！")  # 如果无法成功打开和读取视频，则触发运行时错误
    ...
    #             self.logger.warning(f"temporal sampling failed,file:{sample}")
    #             success_sampled = False
    #     frames_gray_list=np.array(frames_gray_list)
    #     self.logger_cv.info(f"get_snippet {self._path_to_videos[index]}  TIME_cv2_get_snippet {end - start}")
    #
    #     frames_gray_list = np.random.rand(1, 16, 320, 426)
    #
    #     return frames_list, frames_gray_list, start_idx, end_idx, success_sampled

    def _get_snippietsAndframeDif_cv2(self, cap, index):
        """
        用于在一段视频中采样一小段
        *似乎这段代码设计的时候只考虑r=1的情况，启用时要禁用视频重采样
        input:
        ...
        frames_gray_list = np.random.rand(1, 16, 320, 426)
        return frames_list, frames_gray_list, start_idx, end_idx, success_sampled
    def _get_snippietsAndframeDif_decord(self, vr, index):  # 忽略帧差图处理的测试代码
        """
        用于在一段视频中采样一小段
        *似乎这段代码设计的时候只考虑r=1的情况，启用时要禁用视频重采样
        input:
        ...
        self.logger_cv.info(f"get_snippet {self._path_to_videos[index]}  TIME_decord_get_snippet {end - start}")
    ...
    def _get_snippiets_depth(self, index, start_idx, end_idx, frames_gray_list):
        ...
        *似乎这段代码设计的时候只考虑r=1的情况，启用时要禁用视频重采样
        ...
        for i in range(self._repeat_aug):
            ...
            vr_d.seek_accurate(video_start_frame)
...
class VideoMaeDataset(VideoDataset, MaskMapGenerator):
    ...
    def __getitem__(self, index):
        ...
        while not success_sampled:  # 当文件存在且可以打开，但在使用时仍因为不明原因报错时，回到此处重新执行
            # 获取视频文件
            # vr, sample, index = self._get_video_cap(index)
            cap, sample, index =self._get_video_cap_cv2(index)
            # 获取视频采样和帧差图
            # frames_list, frames_gray_list, start_idx, end_idx, success_sampled \
            #     = self._get_snippietsAndframeDif_decord(vr, index)
            frames_list, frames_gray_list, start_idx, end_idx, success_sampled \
                = self._get_snippietsAndframeDif_cv2(cap, index)
        ...
        self.logger_cv.info(f"VIDEO {self._path_to_videos[index]}  TIME {end - start}")
...
def create_dataset(args, mode="pretrain", shuffle=True):
    ...
    if mode in ["pretrain", "finetune"]:
        num_parallel_workers = max(min(args.batch_size * args.repeat_aug * 2, int(cores / args.device_num)), 16)
    else:
    ...
    data = dataset.GeneratorDataset(
        ...
        num_parallel_workers=4 if mode in ["finetune", "pretrain"] else 1,
    ...
    if mode in ["finetune", "pretrain", "pretrain_val"]:
        data = data.map(data_transform_cluser,
                        input_columns="video",
                        num_parallel_workers=4)  # num_parallel_workers
