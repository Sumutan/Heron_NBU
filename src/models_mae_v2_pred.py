...
class PretrainVisionTransformerEncoder(nn.Cell):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        if self.cls_embed:
            self.cls_token = init_params((1, embed_dim), name='cls_token')
            num_patches += 1
        self.dtype = dtype
...
class PretrainVisionTransformer(nn.Cell):
    ...
    def patchify(self, imgs):
        ...
        """
        N, _, T, H, W = imgs.shape  # T num of frame , H、W : 视频宽高
        p = self.patch_embed.patch_size[0]  # p: patch/cube边长
        u = self.t_pred_patch_size  # u ：每个cube涉及的帧数
        assert H == W and H % p == 0 and T % u == 0
        h = w = H // p  # h 、w 为宽高维度上的cube数量
        t = T // u  # t 时间尺度上的cube数量

        x = imgs.reshape(N, 3, t, u, h, p, w, p)
        x = x.transpose(0, 2, 4, 6, 3, 5, 7, 1)  # (N, t, h, w, u, p, p, 3)
        x = x.reshape(N, t * h * w, u * p ** 2 * 3)  # (N, L, C)  (N,1568,1536)
    ...
    def forward_loss_pred(self, imgs, idx, pred, mask):
        """
        imgs: [N, 3, T, H, W]
        ...
        target, patch_info = self.patchify(imgs)        #patch_info = (N, T, H, W, p, u, t, h, w)
    ...
    def construct(self, x, mask, ids_restore, ids_keep, ids_mask):
        ...
        if len(x.shape) == 6:
            b, r, c, t, h, w = x.shape
            x = x.reshape(b * r, c, t, h, w)  # x: b*r, c, t, h, w
        ...
        x = self.decoder(x_full, 0)  # [B, N, 3 * 16 * 16]  0 means return all tokens

        if self.cls_embed:
            x = x[:, 1:, :]  # 去掉cls_token 计算loss

        # loss = self.forward_loss(imgs, x, ids_mask)   #一般模型推理下直接计算loss
        # return loss

        loss, pred_video = self.forward_loss_pred(raw_imgs, ids_mask, x, mask)
        print("loss:",loss)
        # raw_imgs = imgs[:, :, ids_mask, :, :]
        imgs, patch_info = self.patchify(raw_imgs)  # imgs:(N, L, C)  patch_info = (N, T, H, W, p, u, t, h, w)
        N, L, C = imgs.shape
