...
class MaskedAutoencoderViT(nn.Cell):
    def __init__(
        ...
        if self.sep_pos_embed:
            self.pos_embed_spatial = self.init_params((1, input_size[1] * input_size[2], embed_dim), dtype=self.dtype)
            self.pos_embed_temporal = self.init_params((1, input_size[0], embed_dim), dtype=self.dtype)
            if self.cls_embed:
                self.pos_embed_class = self.init_params((1, 1, embed_dim), dtype=self.dtype)
        else:
            if self.cls_embed:
            ...
            self.pos_embed = self.init_params((1, _num_patches, embed_dim), dtype=self.dtype)
