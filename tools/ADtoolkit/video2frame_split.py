...
def dump_frames_simple(vid_item):
    full_path, vid_path, vid_id = vid_item  # full_path=/home/ma-user/work/dataset/TAD/depth/xx.mp4

    print(vid_path)
    # print(full_path)
    vid_name = vid_path.split("/")
    out_full_path = full_path.replace(".mp4", "").replace("/home/ma-user/work/dataset/ucf_crime/video_all",
                                                          "/home/ma-user/work/dataset/ucf_crime_308/frames")
...
def parse_args():
    ...
    parser.add_argument('--src_dir', default="/home/ma-user/work/dataset/ucf_crime/video_all", type=str)  #
    parser.add_argument('--out_dir', default="/home/ma-user/work/dataset/ucf_crime_308/frames", type=str)
...
if __name__ == '__main__':
    ...
    # vid_list = [f for f in vid_list if "Normal_Videos549_x264" in f]
    # fullpath_list = [f for f in fullpath_list if "Normal_Videos549_x264" in f]
    vid_list = [f for f in vid_list if "Normal_Videos308_x264" in f]
    fullpath_list = [f for f in fullpath_list if "Normal_Videos308_x264" in f]

    for e in zip(fullpath_list, vid_list, range(len(vid_list))):
        dump_frames_simple(e)

    # pool = Pool(2)
    # pool.map(dump_frames_simple, zip(fullpath_list, vid_list, range(len(vid_list))))
