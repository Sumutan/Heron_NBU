...
def assign_tasks_avg(tasks, processors=8):
    ...
    processor_times = [0] * processors
    processor_tasks = [[] for _ in range(processors)]
    # task_list = [item[0] for item in tasks]
    # cost_list = [item[1] for item in tasks]
    task_list, cost_list = map(list, zip(*tasks))
    task_range = np.linspace(0, len(cost_list) - 1, processors + 1).astype(int)

    for i in range(processors):
        processor_tasks[i]= task_list[task_range[i]:task_range[i + 1]]
        processor_times[i] = sum(cost_list[task_range[i]:task_range[i + 1]])
...
if __name__ == '__main__':
    ...
    output_dir = "/home/ma-user/work/features/test"
    # task_list = get_video_list_with_framenum_all(input_dir)
    task_list = get_video_list_with_framenum_continue(input_dir, output_dir)

    # task_list, cost_list = map(list, zip(*task_list))
    # task_list=zip(cost_list, cost_list)
    import copy
    task_list2=copy.deepcopy(task_list)

    # task_list = [("任务1", 5), ("任务2", 8), ("任务3", 3), ("任务4", 2), ("任务5", 6)]
    result, load_diff, max_load = assign_tasks(task_list, processors=3)
    print(f"负载最高的 CPU 和负载最低的 CPU 之间的差距为：{load_diff}")
    print(f"CPU最高负载 ：{max_load}")

    result, load_diff, max_load = assign_tasks_avg(task_list2, processors=3)
    print(f"负载最高的 CPU 和负载最低的 CPU 之间的差距为：{load_diff}")
    print(f"CPU最高负载 ：{max_load}")
