...
def dump_frames(vid_item):
    ...
    out_full_path = osp.join("/home/ma-user/work/dataset/shanghaitech/frames_test", vid_name[0].replace(".avi",""))
    os.makedirs(out_full_path,exist_ok=True)
   
    vr = cv2.VideoCapture(full_path)
    videolen = int(vr.get(cv2.CAP_PROP_FRAME_COUNT))
    videolen_true= videolen
    # videolen_true = len(gt_dict[vid_name[0][:-4]])
...
def parse_args():
    ...
    parser.add_argument('--src_dir',default="/home/ma-user/work/dataset/shanghaitech/video", type=str)
    parser.add_argument('--out_dir',default="/home/ma-user/work/dataset/shanghaitech/frames_test",type=str)
