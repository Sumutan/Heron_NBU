...
def buildDataCsv(root_folder_path: str):
    """
    ...
    assert len(classnames) == 101, "类别数！=400"
    classnames.sort()
    ...
    traincsv = []
    for dir2 in dirs2:
        movies = [mov.path.replace("\\", "/") for mov in os.scandir(dir2) if mov.path.endswith('.avi')]
